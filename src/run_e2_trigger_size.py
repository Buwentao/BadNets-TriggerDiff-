...
def main() -> None:
    ...
    p.add_argument("--poison_rate", type=float, default=0.05)
    ...
    rows = []
    for ts in parse_sizes(args.trigger_sizes):
        ...
        print(f"s={ts} CA={ca:.4f} ASR={asr:.4f} ckpt={ckpt_path}")

    df = pd.DataFrame(rows).sort_values("trigger_size")
    out_csv = paths.tables / "e2_trigger_size.csv"
